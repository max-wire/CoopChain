...
var onConfidentialRiskCheck = (runtime, _triggerOutput) => {
  ...
  const eligible = financialSnapshot.creditScore >= MIN_CREDIT_SCORE && financialSnapshot.loanToSavingsRatio <= MAX_LOAN_TO_SAVINGS && financialSnapshot.repaymentRate >= MIN_REPAYMENT_RATE;
  let riskTier;
  if (!eligible) {
    riskTier = "HIGH";
  } else if (financialSnapshot.creditScore < 75 || financialSnapshot.loanToSavingsRatio > 1.5) {
    riskTier = "MEDIUM";
  } else {
    riskTier = "LOW";
  }
