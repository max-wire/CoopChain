...
export class InvestmentToken extends ethereum.SmartContract {
  static bind(address: Address): InvestmentToken {
    return new InvestmentToken("InvestmentToken", address);
  }
  ...
  investmentId(): BigInt {
    let result = super.call("investmentId", "investmentId():(uint256)", []);

    return result[0].toBigInt();
  }

  try_investmentId(): ethereum.CallResult<BigInt> {
    let result = super.tryCall("investmentId", "investmentId():(uint256)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
